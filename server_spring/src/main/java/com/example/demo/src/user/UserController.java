...
//    }
    ...
    @ResponseBody
    @GetMapping("/test/log") // (GET) 127.0.0.1:9000/app/users/:userIdx
    public BaseResponse<List<GetProductRes>> getUser() {
        // @PathVariable RESTful(URL)에서 명시된 파라미터({})를 받는 어노테이션, 이 경우 userId값을 받아옴.
        //  null값 or 공백값이 들어가는 경우는 적용하지 말 것
        //  .(dot)이 포함된 경우, .을 포함한 그 뒤가 잘려서 들어감
        // Get Users
        try {
            List<GetProductRes> getTestResList=userProvider.getImage();
            return new BaseResponse<>(getTestResList);
        } catch (BaseException exception) {
            return new BaseResponse<>((exception.getStatus()));
        }
...
//    }
    ...
    public BaseResponse<TaskRes> logIn(@PathVariable("name") String name) {
        try {
            return new BaseResponse<>(userProvider.getTask(name));
        } catch (BaseException exception) {
            return new BaseResponse<>(exception.getStatus());
        }
    }
...
//    @ResponseBody
//    @PostMapping("/sigh-up")
//    public BaseResponse<User> createUser(@RequestBody PostSighReq postSighReq) {
//        try {
//            return new BaseResponse<>(userService.createUser(postSighReq));
//        } catch (BaseException exception) {
//            return new BaseResponse<>(exception.getStatus());
//        }
//    }
...
//    @ResponseBody
//    @PostMapping("/products/post/pod")
//    public BaseResponse<String> logInPro(@RequestBody PostProductReq postProductReq) {
//        try {
//            userProvider.createProdoucts(postProductReq);
