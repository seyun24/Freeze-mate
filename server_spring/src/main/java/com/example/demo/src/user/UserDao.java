...
public class UserDao {
    ...
    // 회원가입
    public int createUser(PostSighReq postSighReq) {
        String createUserQuery = "insert into User (userName, password) VALUES (?,?)"; // 실행될 동적 쿼리문
        Object[] createUserParams = new Object[]{postSighReq.getUserName(),postSighReq.getPassword()}; // 동적 쿼리의 ?부분에 주입될 값
        ...
        int userId= this.jdbcTemplate.queryForObject(lastInserIdQuery, int.class);

        String createProfileQuery="insert into Profiles(userId, profileName) values(?,?)";
        Object[] profileParams = new Object[]{userId,postSighReq.getProfileName()};
        this.jdbcTemplate.update(createProfileQuery, profileParams);// 해당 쿼리문의 결과 마지막으로 삽인된 유저의 userIdx번호를 반환한다.
        return userId;
    }
...
//        String getPwdQuery = "select userId, userName, password from Professors where userName = ?"; // 해당 email을 만족하는 User의 정보들을 조회한다.
//        String getPwdParams = postLoginReq.getUserName(); // 주입될 email값을 클라이언트의 요청에서 주어진 정보를 통해 가져온다.
//
...
//    }
    ...
    public List<GetTestRes> getTest(){
        String testQuery=" select * from Professors";
        return this.jdbcTemplate.query(testQuery,
    ...
    public List<GetProductRes> getImage(){
        String testQuery=" select productsName, imageLink from Products";
        return this.jdbcTemplate.query(testQuery,
                (rs, rowNum) -> new GetProductRes(
                        rs.getString("productsName"),
                        rs.getString("imageLink")) // RowMapper(위의 링크 참조): 원하는 결과값 형태로 받기
        );
    }

    public void putNumber(int number){
        String numberQuery="insert into Tests(number) values(?)";
        this.jdbcTemplate.update(numberQuery,number);
    }



//    public void createUsers(PostSighReq postSighReq){
//         String createQuery="insert into Users(userName, password) values(?,?)";
//         Object[] createParams = new Object[]{postSighReq.getUserName(), postSighReq.getPassword()};
//         this.jdbcTemplate.update(createQuery,createParams);
//
//         String getQuery="select userId from Users where userName=? and password=?";
//
////         int userId=this.jdbcTemplate.queryForObject(getQuery, int.class);
//         User user=this.jdbcTemplate.queryForObject(getQuery,
//                (rs, rowNum) -> new User(
//                        rs.getInt("userId")
//                ), // RowMapper(위의 링크 참조): 원하는 결과값 형태로 받기
//                createParams
//        );
//         String createProfileQuery="insert into Profiles(userId, profileName) values(?,?)";
//         Object[] profileParams = new Object[]{user.getUserId(), postSighReq.getProfileName()};
//         this.jdbcTemplate.update(createProfileQuery, profileParams);
//    }

//    public void createProduts(PostProductReq postProductReq){
//        String createQuery="insert into Products(productsName, imageLink) values(?, ?)";
//        Object[] createParams = new Object[]{postProductReq.getProductsName(),postProductReq.getImageLink()};
//        this.jdbcTemplate.update(createQuery,createParams);
//    }

    public int logIn(PostLoginReq postLoginReq){
        String logQuery="select userId from Users where userName=? and password=?";
        Object[] logParams= new Object[]{postLoginReq.getUserName(),postLoginReq.getPassword()};
        return this.jdbcTemplate.queryForObject(logQuery,
                int.class,
                logParams);
    }

    public TaskRes getTask(String name){
        String query="select collegeName, collegeEnName, department, location,collegeImg\n" +
                "     ,professorName, ChineseCharName, cellPhone, website,degree, FaxNumber, labPhoneNumber\n" +
                "from Colleges join Labs L on Colleges.collegeId = L.collegeId\n" +
                "join Professors P on Colleges.collegeId = P.collegeId\n" +
                "where professorName=?";
        String query2="select email from Emails\n" +
                "join Professors P on P.professorId = Emails.professorId\n" +
                "where professorName=?";
        List<EmailRes> emailRes = this.jdbcTemplate.query(query2,
                (rs, rowNum) -> new EmailRes(
                        rs.getString("email")),name // RowMapper(위의 링크 참조): 원하는 결과값 형태로 받기
        );

        return this.jdbcTemplate.queryForObject(query,
                (rs, rowNum) -> new TaskRes(
                        rs.getString("collegeName"),
                        rs.getString("collegeEnName"),
                        rs.getString("department"),
                        rs.getString("location"),
                        rs.getString("collegeImg"),
                        rs.getString("professorName"),
                        rs.getString("ChineseCharName"),
                        rs.getString("cellPhone"),
                        rs.getString("website"),
                        rs.getString("degree"),
                        rs.getString("FaxNumber"),
                        rs.getString("labPhoneNumber"),
                        emailRes
                ), // RowMapper(위의 링크 참조): 원하는 결과값 형태로 받기
                name); // 한 개의 회원정보를 얻기 위한 jdbcTemplate 함수(Query, 객체 매핑 정보, Params)의 결과 반환
    }
}
